...
scons target=template_release production=yes use_llvm=yes platform=linuxbsd

# Build windows export templates
scons target=template_debug production=yes use_mingw=yes platform=windows d3d12=no
scons target=template_release production=yes use_mingw=yes platform=windows d3d12=no
